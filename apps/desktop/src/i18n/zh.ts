...
export const zh: Translations = {
  ...
  settings: {
    ...
    mobilePairing: {
      title: 'Hermes 移动伴侣',
      description:
        '将 iPhone 伴侣与此 Hermes 网关配对。手机使用相同的智能体、配置、会话、记忆和技能，不会启动另一个智能体。',
      show: '显示配对码',
      retry: '重试',
      hide: '隐藏配对码',
      refresh: '刷新配对码',
      ...
      publicUrlRequired:
        '请将 dashboard.public_url 设置为能访问此网关的私有 HTTPS 地址，然后重试。不能使用 localhost 或普通 HTTP。',
      tokenRequired: '此连接无法签发可重复使用的本地网关令牌。请切换到使用令牌认证的 Hermes 网关。',
      routeStale:
        '此手机专用地址已无法访问当前 Desktop 网关。刷新只会更新对应的 Tailscale HTTPS 端口，并保留其他 Serve 路由。',
      refreshRoute: '刷新安全路由',
      routeRefreshFailed: 'Hermes 无法刷新此 Tailscale 路由。请确认已安装并连接 Tailscale，然后重试。',
